...
class AVLNode(object):
  ...
  def delete(self):
    """Deletes this node from the AVL.
  ...
  def successor(self):
    ...
    Returns a AVLNode instance, or None if this node has no successor.
    """
    if self.right is not None:
  ...
  def find(self, key):
    ...
    Returns a AVLNode instance with the given key, or None if the key was not
    found.
    """
    if self.key > key:
...
class AVL(object):
  ...
  def find(self, key):
    """ Finds the node corresponding to this key 

    Args:
      key: the key of the node to be found
        
     Returns a BSTNode instance with the given key. If this tree does not
     contain the given key, returns None.
     """
  ...
  def insert(self, key):
    ...
    if self.root is None:
      self.root = node
      return node
    inserted_node = self.root.insert(node)
