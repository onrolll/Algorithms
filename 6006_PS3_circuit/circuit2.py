...
class AVLNode(object):
  ...
  def __init__ (self, kwp):
    self.key = KeyWirePair(kwp.key,kwp.wire)
    self.parent = None
    self.left = None
    self.right = None
    self.height = 0
  ...
  def update_size(self):
    return 1 + (((self.left and self.left.size) or 0) +
                ((self.right and self.right.size) or 0))
  ...
  def successor(self):
    """The node with the next larger key (the successor) in the AVL.
  
  Returns an AVLNode instance, or None if this node has no successor.
  """
    if self.right is not None:
...
class AVL(object):
  ...
  def insert(self, key):
    inserted_node = self._insert(key)
  ...
  def _delete(self,key):
    ...
    if node is self.root:
      pseudo_root = self.node_class(node.key)
  ...
  def _get_balance(node): 
    a = AVL._height(node.left)
    b = AVL._height(node.right) 
...
class TracedRangeIndex(AVL):
  ...
  def list(self, first_key, last_key):
    result = AVL.list(self, first_key, last_key)
...
class CrossVerifier(object):
  ...
  def _events_from_layer(self, layer):
    ...
    # no need for left_edge;
    # left_edge = min([wire.x1 for wire in layer.wires.values()])
    for wire in layer.wires.values():
      if wire.is_horizontal():
        # instead of left_edge use wire.x1 for RangeIndex to contain the correct wires when swept
        self.events.append([wire.x1, 0, wire.object_id, 'add', wire])
        # event[3]='remove' at wire.x2 will ensure RangeIndex only contains relevant wires
        # note event[1] = 2 in order to be sorted after all vertical wires
  ...
  def _compute_crossings(self, count_only):
    ...
    for event in self.events:
      ...
      if event_type == 'add':
        self.index.insert(KeyWirePair(wire.y1, wire))
        ...
        for kwp in self.index.list(KeyWirePairL(wire.y1),
          ...
          # wires in index intersect query wire for sure if between y1 and y2  => no need for next line                      
          #if wire.intersects(kwp.wire):
          cross_wires.append(kwp.key.wire)
        if count_only:
